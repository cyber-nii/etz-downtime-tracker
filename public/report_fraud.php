...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $fraud_type          = in_array($_POST['fraud_type'] ?? '', ['card_fraud','account_takeover','transaction_fraud','internal_fraud','other'])
                           ? $_POST['fraud_type'] : 'other';
    $service_id          = filter_var($_POST['service_id'] ?? null, FILTER_VALIDATE_INT) ?: null;
    ...
    $financial_impact    = filter_var($_POST['financial_impact'] ?? null, FILTER_VALIDATE_FLOAT);
    $financial_impact    = ($financial_impact !== false && $financial_impact >= 0) ? $financial_impact : null;
    $regulatory_reported = isset($_POST['regulatory_reported']) ? 1 : 0;
    $regulatory_details  = trim($_POST['regulatory_details'] ?? '');
    ...
    if (empty($fraud_type)) $errors[] = "Please select a fraud type.";
...
<head>
    ...
    <title>Report Fraud Incident - eTranzact</title>
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 overflow-hidden rounded-xl" style="box-shadow: 0 1px 3px 0 rgba(0,0,0,.05), 0 1px 2px 0 rgba(0,0,0,.03);">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-0.5">
                                    ...
                                    <span class="text-amber-600 dark:text-amber-400 font-medium">Fraud</span>
                                </div>
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Fraud Incident</h3>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-0.5">Log a fraudulent transaction or account compromise</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="POST" enctype="multipart/form-data" class="px-6 pb-8 pt-6 sm:px-8"
                        x-data="{
                            currentStep: 1,
                            stepError: '',
                            regulatoryReported: <?= ($regulatory_reported ?? 0) ? 'true' : 'false' ?>,
                            filePreviews: [],
                            nextStep() {
                                this.stepError = '';
                                if (this.currentStep === 1) {
                                    const ft = document.getElementById('fraud_type').value;
                                    if (!ft) { this.stepError = 'Please select a fraud type.'; return; }
                            ...
                            <div class="flex items-center">
                                <div class="flex flex-col items-center">
                                    <div class="flex items-center justify-center w-9 h-9 rounded-full text-sm font-semibold transition-colors"
                                        :class="currentStep >= 1 ? 'bg-amber-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-500'">
                                    ...
                                    <span class="mt-1.5 text-xs font-medium hidden sm:block"
                                        :class="currentStep >= 1 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400'">What Happened</span>
                                </div>
                                <div class="flex-1 h-0.5 mx-2 transition-colors" :class="currentStep > 1 ? 'bg-amber-500' : 'bg-gray-200 dark:bg-gray-600'"></div>
                                <div class="flex flex-col items-center">
                                    <div class="flex items-center justify-center w-9 h-9 rounded-full text-sm font-semibold transition-colors"
                                        :class="currentStep >= 2 ? 'bg-amber-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-500'">
                                        <template x-if="currentStep > 2"><i class="fas fa-check text-xs"></i></template>
                                        <template x-if="currentStep <= 2"><span>2</span></template>
                                    </div>
                                    <span class="mt-1.5 text-xs font-medium hidden sm:block"
                                        :class="currentStep >= 2 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400'">Impact</span>
                                </div>
                                <div class="flex-1 h-0.5 mx-2 transition-colors" :class="currentStep > 2 ? 'bg-amber-500' : 'bg-gray-200 dark:bg-gray-600'"></div>
                                <div class="flex flex-col items-center">
                                    <div class="flex items-center justify-center w-9 h-9 rounded-full text-sm font-semibold transition-colors"
                                        :class="currentStep >= 3 ? 'bg-amber-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-500'">
                                        <span>3</span>
                                    </div>
                                    <span class="mt-1.5 text-xs font-medium hidden sm:block"
                                        :class="currentStep >= 3 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400'">Response</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="currentStep === 1" class="space-y-6">
                            <p class="text-sm text-gray-500 dark:text-gray-400 -mt-2 pb-2 border-b border-gray-100 dark:border-gray-700">
                                <i class="fas fa-info-circle mr-1"></i> Tell us what type of fraud occurred, which service was affected, and when it happened.
                            </p>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="date" id="incident_date" name="incident_date" required
                                            ...
                                            max="<?= date('Y-m-d') ?>"
                                            class="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm py-2.5 px-3.5 text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500 focus:border-amber-500">
                                    </div>
                                    <div>
                            ...
                            <div>
                                <label for="fraud_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Fraud Type <span class="text-red-500">*</span></label>
                                <select id="fraud_type" name="fraud_type" required
                                    class="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm py-2.5 px-3.5 text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500 focus:border-amber-500">
                                    <option value="">Select fraud type...</option>
                                    <option value="card_fraud" <?= (($_POST['fraud_type'] ?? '') === 'card_fraud') ? 'selected' : '' ?>>Card Fraud</option>
                                    <option value="account_takeover" <?= (($_POST['fraud_type'] ?? '') === 'account_takeover') ? 'selected' : '' ?>>Account Takeover</option>
                                    <option value="transaction_fraud" <?= (($_POST['fraud_type'] ?? '') === 'transaction_fraud') ? 'selected' : '' ?>>Transaction Fraud</option>
                                    <option value="internal_fraud" <?= (($_POST['fraud_type'] ?? '') === 'internal_fraud') ? 'selected' : '' ?>>Internal Fraud</option>
                                    <option value="other" <?= (($_POST['fraud_type'] ?? '') === 'other') ? 'selected' : '' ?>>Other</option>
                                </select>
                            </div>

                            <div>
                                <label for="service_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Affected Service / Channel
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="4"
                                    ...
                                    class="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm py-2.5 px-3.5 text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500 focus:border-amber-500"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
                            </div>
                        </div>
                        ...
                        <div x-show="currentStep === 2" class="space-y-6">
                            <p class="text-sm text-gray-500 dark:text-gray-400 -mt-2 pb-2 border-b border-gray-100 dark:border-gray-700">
                                <i class="fas fa-info-circle mr-1"></i> Estimate the financial impact and severity of this incident.
                            </p>

                            <div>
                                <label for="financial_impact" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Financial Impact (GH₵)
                                    <span class="text-xs text-gray-500 dark:text-gray-400 font-normal ml-2">(Optional)</span>
                                </label>
                                <input type="number" id="financial_impact" name="financial_impact" min="0" step="0.01"
                                    value="<?= isset($_POST['financial_impact']) ? htmlspecialchars($_POST['financial_impact']) : '' ?>"
                                    placeholder="0.00"
                                    class="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm py-2.5 px-3.5 text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500 focus:border-amber-500">
                            </div>
                        ...
                        <div x-show="currentStep === 3" class="space-y-6">
                            <p class="text-sm text-gray-500 dark:text-gray-400 -mt-2 pb-2 border-b border-gray-100 dark:border-gray-700">
                                <i class="fas fa-info-circle mr-1"></i> Document any regulatory reporting and attach supporting evidence.
                            </p>

                            <!-- Regulatory reporting toggle -->
                            <div class="space-y-3">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Reported to Regulatory Body?</label>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <label class="relative flex cursor-pointer rounded-lg border p-4 shadow-sm transition-colors"
                                        :class="regulatoryReported ? 'border-amber-500 ring-2 ring-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 hover:border-amber-400'">
                                        <input type="radio" name="regulatory_reported" value="1" class="sr-only"
                                            @click="regulatoryReported = true" <?= ($regulatory_reported ?? 0) ? 'checked' : '' ?>>
                                        <span class="flex flex-1 flex-col">
                                            <span class="flex items-center gap-2">
                                                <i class="fas fa-check-circle text-amber-500"></i>
                                                <span class="block text-sm font-semibold text-gray-900 dark:text-white">Yes, reported</span>
                                            </span>
                                            <span class="mt-1 block text-xs text-gray-500 dark:text-gray-400">Regulatory body has been notified.</span>
                                        </span>
                                    </label>
                                    <label class="relative flex cursor-pointer rounded-lg border p-4 shadow-sm transition-colors"
                                        :class="!regulatoryReported ? 'border-gray-400 ring-2 ring-gray-400 bg-gray-50 dark:bg-gray-700' : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 hover:border-gray-400'">
                                        <input type="radio" name="regulatory_reported" value="0" class="sr-only"
                                            @click="regulatoryReported = false" <?= !($regulatory_reported ?? 0) ? 'checked' : '' ?>>
                                        <span class="flex flex-1 flex-col">
                                            <span class="flex items-center gap-2">
                                                <i class="fas fa-times-circle text-gray-400"></i>
                                                <span class="block text-sm font-semibold text-gray-900 dark:text-white">Not yet</span>
                                            </span>
                                            <span class="mt-1 block text-xs text-gray-500 dark:text-gray-400">Regulatory reporting is pending.</span>
                                        </span>
                                    </label>
                                </div>
                                <div x-show="regulatoryReported" x-transition>
                                    <label for="regulatory_details" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        Regulatory Details
                                        <span class="text-xs text-gray-500 dark:text-gray-400 font-normal ml-1">(who, when, reference number)</span>
                                    </label>
                                    <textarea id="regulatory_details" name="regulatory_details" rows="3"
                                        placeholder="e.g. Reported to Bank of Ghana on 04 Apr 2026, Reference: BOG-2026-001"
                                        class="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm py-2.5 px-3.5 text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-amber-500 focus:border-amber-500"><?= htmlspecialchars($_POST['regulatory_details'] ?? '') ?></textarea>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="root_cause" name="root_cause" rows="3"
                                    placeholder="How did this fraud occur?"
                                    class="shadow-sm focus:ring-amber-500 focus:border-amber-500 block w-full sm:text-sm border border-gray-300 dark:border-gray-600 rounded-md p-3 bg-white dark:bg-gray-700 dark:text-white"><?= htmlspecialchars($_POST['root_cause'] ?? '') ?></textarea>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md hover:border-amber-400 dark:hover:border-amber-500 transition-colors bg-gray-50 dark:bg-gray-700/50">
                                    <div class="space-y-1 text-center w-full">
                                                        ...
                                                        <div class="flex items-start gap-3">
                                                            ...
                                                            <div class="flex-1 min-w-0">
                                                                ...
                                                                <input type="text" x-model="preview.customName"
                                                                    class="block w-full text-sm border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-1.5 px-2 bg-white dark:bg-gray-800 dark:text-white focus:ring-amber-500 focus:border-amber-500"
                                        ...
                                        <div class="flex text-sm text-gray-600 dark:text-gray-400 justify-center">
                                            <label for="evidence" class="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-amber-600 hover:text-amber-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-amber-500 px-2 py-0.5 border border-amber-600/20">
                                        ...
                                        <p x-show="filePreviews.length > 0" x-text="`${filePreviews.length} file(s) selected`" class="text-sm font-medium text-amber-600 dark:text-amber-400"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-6 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between gap-3 mt-6">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <button x-show="currentStep < 3" type="button" @click="nextStep()"
                                    class="inline-flex items-center py-2 px-5 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-amber-500 hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
