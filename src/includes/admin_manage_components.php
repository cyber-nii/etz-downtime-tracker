    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full">
        ...
        <form method="POST" class="p-6 space-y-4" id="componentForm">
            ...
            <div id="multiNameWrap" class="hidden">
                ...
                <div id="componentNamesList" class="space-y-2">
                    <!-- rows injected by JS -->
                </div>
                <button type="button" onclick="addComponentNameRow()"
                    class="mt-3 inline-flex items-center text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium transition-colors">
                    <i class="fas fa-plus-circle mr-1.5"></i> Add Another
                </button>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-2">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium">
                    <i class="fas fa-save mr-2"></i> <span id="componentSaveLabel">Save</span>
                </button>
            </div>
        </form>
    </div>
...
function addComponentNameRow(value) {
    ...
    row.innerHTML = `
        <input type="text" name="component_names[]" value="${value || ''}"
               placeholder="Component name"
               class="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
        <button type="button" onclick="this.closest('div').remove()" title="Remove"
                class="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors">
