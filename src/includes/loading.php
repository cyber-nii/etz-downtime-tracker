...
<div id="loading-overlay"
    class="fixed inset-0 bg-gray-900 bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50 transition-opacity duration-300"
    style="opacity: 1;">
    <div class="bg-white dark:bg-gray-800 rounded-xl p-8 shadow-2xl transform transition-all duration-300 scale-95"
        id="loading-card">
...
<script>
    ...
    // Failsafe: Never show loading screen for more than 4 seconds automatically
    const fallbackTimer = setTimeout(() => hideLoading(), 4000);

    // Auto-hide loading on DOM ready for snappier experience
    ...
    // Catch full window load
    window.addEventListener('load', function () {
        hideLoading();
        clearTimeout(fallbackTimer);
    });

    // Handle BFCache (Back/Forward Cache) when using browser back button
    window.addEventListener('pageshow', function (event) {
        if (event.persisted) {
            // Page was restored from cache, hide the loader immediately
            hideLoading();
            clearTimeout(fallbackTimer);
        }
    });
