...
DROP TABLE IF EXISTS `incident_company_history`;
CREATE TABLE `incident_company_history` (
  ...
  CONSTRAINT `incident_company_history_ibfk_1` FOREIGN KEY (`incident_id`) REFERENCES `incidents` (`incident_id`) ON DELETE CASCADE,
  CONSTRAINT `incident_company_history_ibfk_2` FOREIGN KEY (`company_id`) REFERENCES `companies` (`company_id`) ON DELETE CASCADE,
  CONSTRAINT `incident_company_history_ibfk_3` FOREIGN KEY (`changed_by`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
...
CREATE TABLE `incident_templates` (
  ...
  `impact_level` enum('low','medium','high','critical') COLLATE utf8mb4_unicode_ci DEFAULT 'medium',
  ...
  CONSTRAINT `incident_templates_ibfk_1` FOREIGN KEY (`service_id`) REFERENCES `services` (`service_id`) ON DELETE SET NULL,
  CONSTRAINT `incident_templates_ibfk_2` FOREIGN KEY (`component_id`) REFERENCES `service_components` (`component_id`) ON DELETE SET NULL,
  CONSTRAINT `incident_templates_ibfk_3` FOREIGN KEY (`created_by`) REFERENCES `users` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `incidents` (
  ...
  `description` text COMMENT 'Detailed description of what happened during the incident',
  ...
  UNIQUE KEY `unique_incident_ref` (`incident_ref`),
  KEY `service_id` (`service_id`),
  KEY `component_id` (`component_id`),
